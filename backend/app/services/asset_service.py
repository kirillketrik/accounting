...
class AssetService:
    ...
    def _next_inventory_number(self, asset_type_id: int, seen: set[str]) -> str:
        """Generate the next free plain-numeric inventory number for this asset type.

        Autogenerated numbers are plain integers ("1", "2", ...), unique among assets
        of the same type, continuing from the highest existing plain-numeric value.
        """
        last_num = 0
        for inv in self.repo.list_inventory_numbers(asset_type_id):
            if inv.isdigit():
                last_num = max(last_num, int(inv))
