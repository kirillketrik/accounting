...
def bulk_create_events(
    payload: AssetEventBulkCreate, db: Session = Depends(get_db)
) -> AssetEventBulkResult:
    return AssetEventService(db).bulk_create_by_inventory_number(payload)


@router.put("/{event_id}", response_model=AssetEventWithType)
def update_event(
    event_id: int, payload: AssetEventUpdate, db: Session = Depends(get_db)
) -> AssetEventWithType:
    return AssetEventService(db).update(event_id, payload)
